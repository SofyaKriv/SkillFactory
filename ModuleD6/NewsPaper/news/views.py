...
def sub_mail(sender, instance, created, **kwargs):
    if created:
        ...
        subject = f'Изменено {instance.text_post}'
    send_mail(
        subject=subject,
        message=f'В Вашей любимой категории новая статья {subject}: ' + instance.text_post + f" http://127.0.0.1:8000/news/{instance.id}",
        from_email='test250621@yandex.ru',
        recipient_list=['test1802@inbox.ru']
    )
...
class PostsCreateView(PermissionRequiredMixin, CreateView):
    ...
    form_class = PostsForm
    success_url = '/news/'
    post_save.connect(sub_mail, sender=Post)
